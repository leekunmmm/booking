...
> File này được trình bày dạng Markdown để dễ đọc trên GitHub. Có thể copy bảng dưới đây paste vào Excel để có file `test-cases.xlsx` .
...
## 3. Test cases — Hồ sơ cá nhân (phụ trách: Nguyễn Lê Duy Hoàng)

| ID    | Module  | Tên                                        | Tiền điều kiện            | Bước thực hiện                                                                                  | Kết quả mong đợi                                               | Mức độ | Trạng thái |
| ----- | ------- | ------------------------------------------ | ------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------ | ---------- |
| TC-31 | Profile | Xem trang hồ sơ cá nhân                   | Đã đăng nhập              | 1. Vào /profile                                                                                 | Hiển thị đúng họ tên, email, số điện thoại, hạng thành viên   | Medium |            |
| TC-32 | Profile | Cập nhật họ tên và số điện thoại           | Đã đăng nhập              | 1. Vào /profile<br>2. Sửa họ tên thành "Nguyễn Văn A", phone="0901234567"<br>3. Lưu            | Thông tin mới được lưu, hiển thị lại đúng                     | Medium |            |
| TC-33 | Profile | Cập nhật email đã tồn tại                  | Đã có account dùng email X | 1. Vào /profile<br>2. Đổi email sang email X đã có<br>3. Lưu                                   | Lỗi "Email đã được sử dụng"                                   | Medium |            |
| TC-34 | Profile | Upload ảnh đại diện hợp lệ                 | Đã đăng nhập              | 1. Vào /profile<br>2. Chọn file ảnh .jpg < 10MB<br>3. Lưu                                      | Ảnh mới xuất hiện trên navbar và trang profile                | Medium |            |
| TC-35 | Profile | Upload file không phải ảnh                 | Đã đăng nhập              | 1. Vào /profile<br>2. Chọn file .pdf hoặc .exe<br>3. Lưu                                       | Lỗi hoặc không chấp nhận file                                 | Low    |            |
| TC-36 | Profile | Truy cập /profile khi chưa đăng nhập       | Chưa đăng nhập            | 1. GET /profile                                                                                 | Redirect /login?redirect=/profile                             | High   |            |
| TC-37 | Auth    | Đăng ký với confirmPassword không khớp    | –                         | 1. /register<br>2. password="abc123", confirmPassword="abc456"<br>3. Submit                     | Lỗi "Mật khẩu xác nhận không khớp"                            | Medium |            |
| TC-38 | Auth    | Đăng ký họ tên để trống                    | –                         | 1. /register<br>2. Để trống trường họ tên<br>3. Submit                                          | Lỗi "Họ tên không được để trống"                              | Medium |            |
| TC-39 | Auth    | Đăng ký username < 3 ký tự                 | –                         | 1. /register username="ab"                                                                      | Lỗi "Tên đăng nhập phải có ít nhất 3 ký tự"                  | Medium |            |
| TC-40 | Auth    | Đăng nhập rồi truy cập lại /login          | Đã đăng nhập              | 1. Đang login<br>2. Vào /login trực tiếp                                                        | Redirect về trang chủ (không cho vào lại login)               | Low    |            |
